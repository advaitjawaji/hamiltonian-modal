...
class RoboScapeAdapter:
    ...
    RoboScape is a large-scale visuomotor policy trained on diverse
    robot manipulation and locomotion data.  This adapter provides the
    standard benchmarking interface.
    ...
    def __init__(self, env_name: str = "g1_walking", seed: int = 42) -> None:
        ...
        self._step = 0
        logger.info("RoboScapeAdapter initialised (env=%s, seed=%d)", env_name, seed)
    ...
    def predict(self, obs: NDArray[np.float64]) -> NDArray[np.float64]:
        ...
        """
        self._step += 1
        return 0.35 * self._rng.standard_normal(23).astype(np.float64)

    def synthetic_reward_curve(self, n_steps: int) -> NDArray[np.float64]:
        """Simulated RoboScape reward curve.
        ...
        curve = 0.60 * (1.0 - np.exp(-t / (n_steps * 0.35)))
    ...
    def synthetic_drift_trajectory(self, n_steps: int) -> NDArray[np.float64]:
        """Simulated energy drift for RoboScape.
        ...
        return synthetic_baseline_drift(n_steps, drift_rate=0.35, seed=int(self._rng.integers(0, 1000)))
